...
from fairseq.models.wav2vec.wav2vec import Wav2VecPredictionsModel, ConvAggegator
from fairseq.models.wav2vec.mlp import MLPFeatureExtractionModel
...
@dataclass
class Wav2Vec_MLP_Config(FairseqDataclass):
    input_size: int = field(
        default=82, metadata={'help': 'number of input channels'}
    )
    hidden_sizes: str = field(
        default="", metadata={'help': 'list of hidden sizes'}
    )
    output_size: int = field(
        default=10, metadata={'help': 'output size of encoder'}
    )
...
@register_model("wav2vec_mlp", dataclass=Wav2Vec_MLP_Config)
class Wav2Vec_MLP_Model(BaseFairseqModel):
    @classmethod
    def build_model(cls, cfg: Wav2Vec_MLP_Config, task: FairseqTask):
        """Build a new model instance."""

        model = Wav2Vec_MLP_Model(cfg)
        return model

    def __init__(self, cfg: Wav2Vec_MLP_Config):
        super().__init__()
        ...
        hidden_sizes = eval(cfg.hidden_sizes)
        self.feature_extractor = MLPFeatureExtractionModel(
            cfg.input_size, hidden_sizes, cfg.output_size
        )
        ...
        if cfg.vq_type == "gumbel":
            self.vector_quantizer = GumbelVectorQuantizer(
                ...
                vq_dim=cfg.vq_dim if cfg.vq_dim > 0 else cfg.output_size,
                time_first=False,
